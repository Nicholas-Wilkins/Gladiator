"""
SQLite persistence for Gladiator.
...
class DB:
    def __init__(self, path: str | Path = "gladiator.db") -> None:
        self.path = Path(path)
    ...
    def save_champion(self, bot: Bot, state) -> None:
        ...
        # Log each new champion (avoid duplicate entries for same bot)
    ...
    def save_match(self, result: MatchResult, state) -> None:
        ...
        # Determine champion vs challenger from match result
        # The champion is the bot that was champion before this match.
        # We identify them by their bot_id stored in state.champion.
        champion_id = state.champion.bot_id

        # Find the challenger id: it's whichever of winner/loser isn't the champion.
        if result.winner_id == champion_id:
            ...
            champion_won = 0
            # Score is symmetric; we just record both totals
    ...
    # ------------------------------------------------------------------ #
    # Queries for the dashboard / inspection                               #
    # ------------------------------------------------------------------ #
...
def _is_bot_a(result: MatchResult, bot_id: str) -> bool:
    """True if bot_id was bot_a (white in game 1 of first pair)."""
    if not result.pairs:
        return True
    g1 = result.pairs[0].game1
    # bot_a is white in game 1; if the game's white won and that winner matches, bot_a won
    # We can't directly tell, so we use the score fields which are labelled a/b.
    # The champion can be either a or b; just check winner in game1.
