...
def play_game(white: Bot, black: Bot, move_cb=None, stop_check=None) -> GameResult:
    """Play a single game; returns a GameResult.

    move_cb, if provided, is called as move_cb(board, move, white_id, black_id)
    after every half-move with a *copy* of the board after the move.
    """
    board = chess.Board()
    game = chess.pgn.Game()
...
def play_pair(bot_a: Bot, bot_b: Bot, move_cb=None, stop_check=None) -> PairResult:
    """
...
def play_match(bot_a: Bot, bot_b: Bot, rng=None, progress_cb=None, move_cb=None, abort_check=None) -> MatchResult:
    """
    ...
    while True:
        ...
        if pair_num >= _MAX_PAIRS:
            # Genuinely tied after max pairs — random tiebreak
            if (rng.random() if rng is not None else _random.random()) < 0.5:
                result.total_score_a += 0.001  # nudge to pick a winner
            else:
