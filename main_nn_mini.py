"""
Gladiator-NN-Mini — GPU-accelerated chess engine with a lower mutation threshold.

Identical to the full Gladiator-NN engine but enters MUTATION mode after 15
consecutive wins instead of 100, allowing mutation phase to be reached faster.

Usage
-----
    python main_nn_mini.py                   # train with dashboard (gladiator_nn_mini.db)
    python main_nn_mini.py --no-ui           # headless
    python main_nn_mini.py --db other.db     # custom database path
    python main_nn_mini.py --seed 42         # deterministic RNG seed
    python main_nn_mini.py --cpu             # force CPU even if GPU is available
    python main_nn_mini.py --history         # print champion lineage and exit
    python main_nn_mini.py --stats           # print recent match stats and exit
    python main_nn_mini.py --max-games 500   # stop after N games (testing)
"""
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Gladiator-NN-Mini chess engine trainer")
    p.add_argument("--db", default="gladiator_nn_mini.db", help="SQLite database path")
...
def _print_history(db) -> None:
    ...
    table = Table(title="Champion Lineage (NN-Mini)", show_lines=True)
...
def _print_stats(db) -> None:
    ...
    table = Table(title="Recent Match Results — NN-Mini (last 30)", show_lines=False)
...
def main() -> None:
    ...
    from gladiator_nn_mini.training.trainer import Trainer
    ...
    from gladiator_nn_mini.ui.dashboard import Dashboard
