"""
SQLite persistence for Gladiator-NN.

Schema is identical to the CPU version; the difference is that params_json
holds base64-encoded PyTorch weights instead of heuristic parameter dicts.
"""
...
class DB:
    def __init__(self, path: str | Path = "gladiator_nn.db") -> None:
        self.path = Path(path)
    ...
    def load_champion(self) -> Optional[Tuple[NNBotParams, dict]]:
        """Return (NNBotParams, meta_dict) or None.  Caller wraps in NNBot(params, device)."""
        row = self._con.execute("SELECT * FROM champion WHERE id = 1").fetchone()
        if row is None:
            return None
        params = NNBotParams.from_json(row["params_json"])
        meta = {
    ...
    def save_match(self, result: MatchResult, state) -> None:
        ...
        for pair_num, pair in enumerate(result.pairs, start=1):
            for game_in_pair, game in enumerate([pair.game1, pair.game2], start=1):
                white_id = champion_id if game_in_pair == 1 else challenger_id
                black_id = challenger_id if game_in_pair == 1 else champion_id
                self._con.execute(
                    """
                    INSERT INTO games
    ...
    def max_streak(self) -> int:
        """Longest consecutive-win streak ever, derived from champion_log gaps."""
        log = self._con.execute(
            "SELECT total_matches_at_crowning FROM champion_log ORDER BY rowid"
        ).fetchall()
        champ = self._con.execute(
            "SELECT consecutive_wins FROM champion WHERE id = 1"
        ).fetchone()

        if not log:
            return champ[0] if champ else 0
