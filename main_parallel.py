...
def main() -> None:
    ...
    with Live(_display(), console=console, refresh_per_second=2, screen=True) as live:
        try:
            while True:
                ...
                # Keep running maximum — captures streaks before they reset on dethronement
    ...
    if winner_idx is not None:
        ...
        console.print(
            ...
            "Run [bold]`python main.py`[/bold] to resume training in MUTATION mode."
        )
    else:
        console.print("\nNo winner promoted. Worker DBs preserved for inspection.")
        console.print("Re-run [bold]`python main_parallel.py`[/bold] to resume parallel exploration.")
