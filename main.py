"""
Gladiator — self-improving chess engine.

Usage
-----
    python main.py                  # start / resume training with dashboard
    python main.py --no-ui          # headless (logs to stdout only)
    python main.py --seed 42        # deterministic RNG seed
    python main.py --db path/to.db  # custom database path
    python main.py --history        # print champion lineage and exit
    python main.py --stats          # print recent match stats and exit
"""
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Gladiator chess engine trainer")
    p.add_argument("--db", default="gladiator.db", help="SQLite database path")
...
def _print_history(db) -> None:
    ...
    table = Table(title="Champion Lineage", show_lines=True)
...
def _print_stats(db) -> None:
    ...
    table = Table(title="Recent Match Results (last 30)", show_lines=False)
...
def main() -> None:
    ...
    # --- Rich dashboard mode ---
    from gladiator.ui.dashboard import Dashboard
