"""
Rich terminal dashboard for Gladiator-NN-Mini training.
"""
...
from gladiator_nn_mini.bot.network import HIDDEN_DIMS, INPUT_DIM
from gladiator_nn_mini.training.match import MatchResult
from gladiator_nn_mini.training.trainer import Mode, TrainerState, RANDOM_WIN_THRESHOLD
...
def _render_header(state: Optional[TrainerState], best_streak: int) -> Panel:
    if state is None:
        return Panel("Waiting for first match…", title="Gladiator-NN-Mini", border_style="dim")
    ...
    return Panel(grid, title="[bold white]GLADIATOR-NN-MINI[/bold white]", border_style="cyan", padding=(0, 1))
...
def _render_champion(state: Optional[TrainerState]) -> Panel:
    ...
    needs = max(0, RANDOM_WIN_THRESHOLD - state.consecutive_wins) if state.mode == Mode.RANDOM else 0
    streak_text = (
        f"Streak: {state.consecutive_wins}/{RANDOM_WIN_THRESHOLD} random wins"
